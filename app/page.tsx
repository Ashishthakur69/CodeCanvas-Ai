...
export default function CodeCanvasPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: uuidv4(),
      role: "ai",
      content: "Hello! I'm CodeCanvas AI. Describe the component you want to build and select your target framework above.",
      timestamp: new Date(),
      code: INITIAL_CODE,
    },
  ])
  ...
  const handleSendMessage = async (prompt: string, imageFile: File | null) => {
    ...
    try {
      ...
      while (true) {
        ...
        handleFileContentChange(activeFileId, streamedCode);
        setMessages(prev => prev.map(msg => 
          msg.id === aiMessageId ? { 
            ...msg, 
            content: streamedCode.substring(0, 100) + (streamedCode.length > 100 ? '...' : '') 
          } : msg
        ));
      }
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === aiMessageId ? { 
          ...msg, 
          content: `Code generated for ${selectedFramework}. View full code in editor.`,
          code: streamedCode
        } : msg
      ));
      toast.success(`Code generation complete for ${selectedFramework}!`, { duration: 2000 });

    } catch (error) {
      console.error(error);
      toast.error("An error occurred during AI generation. Check console.");
      setMessages(prev => prev.map(msg => 
        msg.id === aiMessageId ? { 
          ...msg, 
          content: 'Error: Could not generate code. Check server logs.',
        } : msg
      ));
    } finally {
      setIsGenerating(false);
  ...
  const handleFormatCode = () => {
    toast.info("Formatting not enabled for MVP yet.")
  }
  
  const handleRegenerateMessage = (messageId: string) => {
    toast.info("Regenerate not enabled for MVP. Try a new prompt.", { duration: 2000 })
  }
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      ...
      {/* Main 3-Panel Layout - RESIZABLE */}
      <main className="flex flex-1 overflow-hidden">
        <PanelGroup direction="horizontal">
          
          {/* Left Panel - Chat UI (Input) */}
          <Panel defaultSize={20} minSize={20} maxSize={30} className="w-full max-w-sm">
          ...
          <Panel defaultSize={40} minSize={30}>
            <section className="flex h-full flex-col bg-[var(--color-panel-preview)]">
              <LivePreview code={activeFile.content}framework={selectedFramework} />
            </section>
          </Panel>
